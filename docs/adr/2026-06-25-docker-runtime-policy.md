...
- The `Provider Image Publish` GitHub Actions workflow builds and validates the provider image in CI, logs in to GHCR with `GITHUB_TOKEN`, and runs the same `docker:publish-provider --apply` path with repository `packages: write` permissions when local package-push credentials are unavailable.
