...
- `pnpm ops docker:publish-provider -- --target-tag <registry>/<repo>:<tag>` validates the source provider image, prints the exact `docker tag` and `docker push` commands, and records the accepted runtime credential env/volume allowlist. It is dry-run-first; pushing requires `--apply --approved-by <operator>`.
