...
- Use `pnpm ops claim-locks` to audit distributed GitHub lock refs. Add `--json` for machine-readable stdout or `--output .ai/claim-locks.json` for a monitor/dashboard snapshot. The `Claim Lock Audit` GitHub Actions workflow runs the JSON audit on a schedule and uploads the report artifact. Apply mode deletes orphaned refs only; stale matched refs should go through `reclaim-expired --apply-lock-ref`.
