...
- `pnpm ops claim -- --dispatch docs/agents/dispatches/dispatch-id.json --claimed-by runner-name --lease-hours 24`: claim with explicit lease expiry metadata; omitted lease hours default to 24
...
- `pnpm ops schedule -- --queue .ai/queue.json --dispatch --require-lock-ref`: create dispatch artifacts that require a GitHub ref claim lock when claimed
- `pnpm ops github:init`: print a dry-run GitHub Tracker Bootstrap report, including live ProjectV2 view-mutation capability from the GitHub GraphQL schema when authenticated
...
- `pnpm ops github:init -- --apply --create-project-views`: create missing recommended GitHub Project views through the REST Project views API after inspecting existing views
...
- `pnpm ops run -- --dispatch docs/agents/dispatches/dispatch-id.json --execute --execute-docker`: launch the rendered Docker command after inspection; add `--live-provider` when the container image contains the selected authenticated provider CLI
- `pnpm ops docker:validate -- --image node:22-bookworm --provider codex --require-command node,pnpm,git --docker-env CODEX_HOME`: run a no-network readiness probe for a proposed Docker execution image, required commands, selected provider CLI, and explicit credential env allowlist
- `pnpm ops docker:build-provider -- --tag autopocock-provider-runner:local --validate`: build the repo-owned provider runner image with pinned Codex and Claude Code CLIs, then run offline command/version validation
- `pnpm ops docker:publish-provider -- --source-tag autopocock-provider-runner:local --target-tag ghcr.io/OWNER/autopocock-provider-runner:TAG --credential-env CODEX_HOME --write-plan`: validate the source provider image, print the exact registry tag/push commands, and write a durable runtime credential allowlist plan; add `--apply --approved-by <operator>` only after the registry/tag and credential package are accepted
- `Provider Image Publish` GitHub Actions workflow: manually dispatch a CI-backed GHCR publish using repository `packages: write` permissions after choosing image name/tag and credential env/volume allowlists
...
- `pnpm ops github:init` is a bootstrap report by default. With `-- --apply`, it creates missing canonical labels. Add `--create-project` for a fresh Project when no Project reference is configured, add `--create-project-fields` to create missing configured Project fields, add `--update-project-fields` to repair supported Project field option-name drift, and add `--create-project-views` to create missing recommended views through REST.
- GitHub Project view creation is available through the REST Project views API. Existing view rename/delete remains manual while the supported APIs inspected here expose no view rename/delete operation; use `pnpm ops github:init -- --write-view-plan` to generate an exact workaround artifact when drift remains.
...
- Claim dispatches with a stable runner identity using `pnpm ops claim`; the command uses an exclusive local dispatch-artifact lock, re-reads state before mutation, and records a 24-hour lease by default. Use `--lease-hours` when the runner should have a shorter or longer explicit expiry. Add `--apply-tracker` when the dispatch came from a GitHub-exported queue and you want the Project `Runner` field to show the live claim. Add `--apply-lock-ref` when multiple machines may claim from the same dispatch pool; it atomically creates a GitHub ref and fails duplicate claims before local state changes. Scheduler-created dispatches can require that behavior by using `pnpm ops schedule -- --dispatch --require-lock-ref`, which makes `claim` acquire the GitHub ref lock automatically. For worktree isolation, keep the derived or explicit `worktree_path` available locally.
...
- Use `pnpm ops docker:build-provider -- --tag autopocock-provider-runner:local --validate` to build the default provider image from `docker/provider-runner/Dockerfile`. Use `pnpm ops docker:publish-provider -- --target-tag <registry>/<repo>:<tag> --credential-env <NAME> --write-plan` to validate the image and produce an exact production tag/push plan under `docs/agents/hitl/`; add `--apply --approved-by <operator>` only after `docker login`, registry/tag, and credential package are accepted. When local package-push auth is unavailable, dispatch the `Provider Image Publish` GitHub Actions workflow instead; it builds and validates the provider image in CI, logs in to GHCR with `GITHUB_TOKEN`, and calls the same `docker:publish-provider --apply` path with the chosen image name/tag and credential allowlists. Use `pnpm ops docker:validate -- --image <image> --provider <codex|claude> --require-command node,pnpm,git --docker-env <NAME>` before relying on any other Docker execution image for live work. Then use `pnpm ops dispatch -- --isolation-mode docker --docker-image autopocock-provider-runner:local ...` or claim an existing Docker dispatch, and run `pnpm ops run -- --prepare-docker` to inspect the exact Docker command. Launch it with `pnpm ops run -- --execute --execute-docker` only after the container boundary is acceptable. Credentials are never forwarded implicitly; use `--docker-env` for allowed env vars and `--docker-volume` for extra writable mounts. Add `--live-provider` only when the image has the selected provider CLI and credentials. Docker execution labels managed containers so `pnpm ops docker:clean` can dry-run/apply cleanup for stale stopped AutoPocock containers without touching user-declared volumes.
...
- Use `pnpm ops claim-locks` to audit distributed GitHub lock refs. Add `--json` for machine-readable stdout or `--output .ai/claim-locks.json` for a monitor/dashboard snapshot. The `Claim Lock Audit` GitHub Actions workflow runs the JSON audit on a schedule, writes a run-summary dashboard, and uploads the report artifact. Apply mode deletes orphaned refs only; stale matched refs should go through `reclaim-expired --apply-lock-ref`.
