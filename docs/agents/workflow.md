...
- `pnpm ops docker:publish-provider -- --source-tag autopocock-provider-runner:local --target-tag ghcr.io/OWNER/autopocock-provider-runner:TAG --credential-env CODEX_HOME`: validate the source provider image, print the exact registry tag/push commands, and document the runtime credential allowlist; add `--apply --approved-by <operator>` only after the registry/tag and credential package are accepted
