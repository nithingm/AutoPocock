...
- `pnpm ops github:init -- --write-view-plan`: write a Prepared Human Step with exact Project view create/rename actions from the latest inspection
...
- GitHub Project view creation and renaming are still manual because GitHub does not expose ProjectV2 view mutations; use `pnpm ops github:init -- --write-view-plan` to generate the exact manual workaround artifact and verification command.
