...
- Treat `pnpm ops github:init` as a bootstrap and drift report. With `-- --apply`, it creates missing labels; with `-- --apply --create-project --project-title "Name"`, it creates a fresh Project only when no Project reference is configured; with `-- --apply --create-project-fields`, it creates missing configured Project fields; with `-- --apply --update-project-fields`, it repairs supported single-select Project field option-name drift. It inspects recommended Project views, reports missing views or name drift, and checks the live GraphQL mutation schema; GitHub still requires manual view creation or renaming while no ProjectV2 view mutation is exposed.
