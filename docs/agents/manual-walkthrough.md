...
- Treat `pnpm ops github:init` as a bootstrap and drift report. With `-- --apply`, it creates missing labels; with `-- --apply --create-project --project-title "Name"`, it creates a fresh Project only when no Project reference is configured; with `-- --apply --create-project-fields`, it creates missing configured Project fields; with `-- --apply --update-project-fields`, it repairs supported single-select Project field option-name drift; with `-- --apply --create-project-views`, it creates missing recommended Project views through REST. It inspects recommended Project views, reports missing views or name drift, and checks the live GraphQL mutation schema; GitHub still requires manual view rename/delete while no supported API exposes those operations.
