...
- `github:init` remains dry-run-first and reports existing label and Project field drift before mutation. The guarded `--apply --create-project` path now creates a fresh Project only when no Project reference is configured, the explicit `--apply --create-project-fields` path created the missing configured optional Project fields on Project 1, and `--apply --update-project-fields` repaired the supported `Execution Stage` option-name drift.
- `github:init` now inspects the live GitHub GraphQL mutation schema for ProjectV2 view mutation capability and uses the REST Project views API for missing recommended view creation. The latest live report says all recommended Project views are present with no planned view changes.
- `github:init -- --write-view-plan` writes a Prepared Human Step for exact Project view workarounds with the same schema capability evidence. The previous leading-space name drift ` Validation` -> `Validation` was resolved for the recommended view set by creating an exact `Validation` view through REST. Legacy extra views can only be renamed or deleted manually because the supported APIs inspected here expose no view rename/delete operation.
...
The configured optional Project fields `Review Capacity Cost`, `Runner`, `Last Scheduler Plan`, and `PR` were created through `pnpm ops github:init -- --apply --create-project-fields`. The previous live `Execution Stage` option spelling drift (`Ready To Slice` versus `Ready to Slice`) was repaired through `pnpm ops github:init -- --apply --update-project-fields`; bootstrap now reports all configured Project fields as present. The exact recommended `Validation` Project view was created through `pnpm ops github:init -- --apply --create-project-views`; bootstrap now reports all recommended Project views as present.
