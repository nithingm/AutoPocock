...
- Dispatch Claims use local filesystem locks around claim/reclaim mutations. Claims record explicit lease expiry metadata, `claim --apply-tracker` publishes the runner lease to the GitHub Project `Runner` field for external visibility, and `claim --apply-lock-ref` can acquire an atomic GitHub branch ref under `refs/heads/autopocock-locks/` before local mutation. `schedule --dispatch --require-lock-ref` marks scheduler-created dispatches so `claim` acquires that GitHub ref lock automatically. `reclaim --apply-lock-ref` and `reclaim-expired --apply-lock-ref` release those distributed lock refs during approved recovery. `claim-locks --json` and `claim-locks --output` expose the distributed lock audit as a machine-readable monitor/dashboard contract, and the `Claim Lock Audit` GitHub Actions workflow runs that audit on a schedule with a run-summary dashboard.
...
- How to package the distributed GitHub ref lock path for always-on multi-runner deployments beyond the landed scheduler dispatch policy, `claim-locks` audit/cleanup, JSON report contract, scheduled audit workflow, and Actions run-summary dashboard, including external operator dashboards if needed.
