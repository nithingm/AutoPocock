...
The `Claim Lock Audit` GitHub Actions workflow runs the distributed lock audit on a schedule, writes a run-summary dashboard, and uploads `claim-locks.json`.
...
- Run `pnpm ops github:init` first as a dry run. The validated behavior is bootstrap reporting plus missing-label creation with `-- --apply`, fresh Project creation with `-- --apply --create-project --project-title "Name"` when no Project reference is configured, missing configured Project field creation with `-- --apply --create-project-fields`, supported Project field option-name drift repair with `-- --apply --update-project-fields`, and missing recommended Project view creation with `-- --apply --create-project-views`.
- Create or connect the GitHub Project deliberately. `github:init` refuses to create a duplicate Project when `.ai/ops.config.json` already has a Project URL, ID, or number. It inspects recommended Project views through GraphQL and reports missing views or name drift. Missing recommended views can be created through REST; existing view rename/delete remains manual while GitHub exposes no supported ProjectV2 view mutation for that operation.
...
- external distributed lock dashboards beyond the GitHub Actions run summary
